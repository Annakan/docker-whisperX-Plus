...
RUN --mount=source=load_align_model.py,target=load_align_model.py \
    for i in ${WLANG}; do echo "Preload align model: $i"; python3 load_align_model.py "$i"; done
...
ENTRYPOINT [ "dumb-init", "--", "/bin/sh", "-c", "WLANG=$(echo ${WLANG} | cut -d ' ' -f1); whisperx --model \"${WHISPER_MODEL}\" --language \"${WLANG}\" \"$@\"" ]
